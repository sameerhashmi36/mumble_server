...
void Server::msgAuthenticate(ServerUser *uSource, MumbleProto::Authenticate &msg) {
	...
	while (!q.isEmpty()) {
		...
		foreach (c, c->qlChannels) {
			bool hca = ServerDB::hasChannelAccess(uSource->iId, c->iId);
			if(hca){
				q.enqueue(c);
			}
...
void Server::msgChannelState(ServerUser *uSource, MumbleProto::ChannelState &msg) {
	...
	if (msg.has_name()) {
		...
		// ********* helping for not creating same name channel inside root channel. hint: sam
		if (p || (c && c->iId != 0)) {
...
//		}
		...
		// **************** sending channel id and user id in channel access func.
		c = addChannel(p, qsName, msg.temporary(), msg.position(), msg.max_users());
		if(!c->bTemporary){
			Channel_Access(c->iId,uSource->iId);
//			printf("&&&&&&&&&&&&&&&&&&&&%d&&&&&&&&&&&& %d\n", c->iId, uSource->iId);
		}

...
ServerDB::ServerDB() {
	...
	if (version < DB_STRUCTURE_VERSION) {
		...
		if (Meta::mp.qsDBDriver == "QSQLITE") {
			...
			SQLDO("CREATE TABLE `%1channels` (`server_id` INTEGER NOT NULL, `channel_id` INTEGER NOT NULL, `parent_id` "
				  "INTEGER, `name` varchar(255), PRIMARY KEY (channel_id), `inheritacl` INTEGER) ENGINE=InnoDB");
			SQLDO("CREATE UNIQUE INDEX `%1channel_id` ON `%1channels`(`server_id`, `channel_id`)");
...
void Server::Channel_Access(int channel_id, int user_id){
	TransactionHolder th;
	...
	SQLPREP("INSERT INTO `%1channels_access` (`channel_id`, `user_id`) VALUES (?, ?)");
//	query.addBindValue(id);
	...
	SQLPREP(
		"UPDATE `%1channels` SET `user_id` = ? WHERE `channel_id` = ? ");
//	query.addBindValue(id);
	query.addBindValue(user_id);
	query.addBindValue(channel_id);
	printf("****************2***************\n");
...
bool ServerDB::hasChannelAccess(int user_id, int channel_id) {
	...
	SQLPREP("SELECT `channel_access_id` FROM `%1channels_access` WHERE `channel_id`=? AND `user_id`=?");
	query.addBindValue(channel_id);
	query.addBindValue(user_id);
	SQLEXEC();
